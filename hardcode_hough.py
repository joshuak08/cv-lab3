...
def find_hough_circles(image, r_min, r_max, delta_r, num_thetas, bin_threshold, post_process = True):
  ...
  rs = np.arange(r_min, r_max, step=1)
  ...
  print('finished adding circle candidates')
...
def main():
    ...
    input_img = cv2.imread('rewrite_mag.png', cv2.IMREAD_GRAYSCALE)
    ...
    circle_img, circles = find_hough_circles(input_img, r_min, r_max, delta_r, num_thetas, bin_threshold)
